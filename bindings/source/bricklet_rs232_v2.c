...
static void rs232_v2_callback_wrapper_read_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + RS232_V2_CALLBACK_READ_LOW_LEVEL];
...
static void rs232_v2_callback_wrapper_error_count(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + RS232_V2_CALLBACK_ERROR_COUNT];
...
void rs232_v2_create(RS232V2 *rs232_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(rs232_v2, uid, ipcon->p, 2, 0, 0);
