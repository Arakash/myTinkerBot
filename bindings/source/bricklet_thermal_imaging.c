...
static void thermal_imaging_callback_wrapper_high_contrast_image_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_HIGH_CONTRAST_IMAGE_LOW_LEVEL];
	HighContrastImageLowLevel_Callback *callback = (HighContrastImageLowLevel_Callback *)packet;
...
static void thermal_imaging_callback_wrapper_temperature_image_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + THERMAL_IMAGING_CALLBACK_TEMPERATURE_IMAGE_LOW_LEVEL];
	int i;
	uint16_t aligned_image_chunk_data[31];

	TemperatureImageLowLevel_Callback *callback = (TemperatureImageLowLevel_Callback *)packet;
...
void thermal_imaging_create(ThermalImaging *thermal_imaging, const char *uid, IPConnection *ipcon) {
	...
	device_create(thermal_imaging, uid, ipcon->p, 2, 0, 0);
