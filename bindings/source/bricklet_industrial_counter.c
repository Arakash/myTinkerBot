...
static void industrial_counter_callback_wrapper_all_counter(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_COUNTER_CALLBACK_ALL_COUNTER];
	int i;
	int64_t aligned_counter[4];

	AllCounter_Callback *callback = (AllCounter_Callback *)packet;
...
static void industrial_counter_callback_wrapper_all_signal_data(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + INDUSTRIAL_COUNTER_CALLBACK_ALL_SIGNAL_DATA];
	int i;
	...
	AllSignalData_Callback *callback = (AllSignalData_Callback *)packet;
...
void industrial_counter_create(IndustrialCounter *industrial_counter, const char *uid, IPConnection *ipcon) {
	...
	device_create(industrial_counter, uid, ipcon->p, 2, 0, 0);
