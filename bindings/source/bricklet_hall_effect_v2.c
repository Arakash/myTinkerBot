...
static void hall_effect_v2_callback_wrapper_magnetic_flux_density(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + HALL_EFFECT_V2_CALLBACK_MAGNETIC_FLUX_DENSITY];
	MagneticFluxDensity_Callback *callback = (MagneticFluxDensity_Callback *)packet;
...
static void hall_effect_v2_callback_wrapper_counter(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + HALL_EFFECT_V2_CALLBACK_COUNTER];
	Counter_Callback *callback = (Counter_Callback *)packet;
...
void hall_effect_v2_create(HallEffectV2 *hall_effect_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(hall_effect_v2, uid, ipcon->p, 2, 0, 0);
