...
static void uv_light_v2_callback_wrapper_uva(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + UV_LIGHT_V2_CALLBACK_UVA];
	UVA_Callback *callback = (UVA_Callback *)packet;
...
static void uv_light_v2_callback_wrapper_uvb(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + UV_LIGHT_V2_CALLBACK_UVB];
	UVB_Callback *callback = (UVB_Callback *)packet;
...
static void uv_light_v2_callback_wrapper_uvi(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + UV_LIGHT_V2_CALLBACK_UVI];
	UVI_Callback *callback = (UVI_Callback *)packet;
...
void uv_light_v2_create(UVLightV2 *uv_light_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(uv_light_v2, uid, ipcon->p, 2, 0, 0);
