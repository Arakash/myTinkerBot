...
static void nfc_callback_wrapper_reader_state_changed(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + NFC_CALLBACK_READER_STATE_CHANGED];
	bool unpacked_idle;
	ReaderStateChanged_Callback *callback = (ReaderStateChanged_Callback *)packet;
...
static void nfc_callback_wrapper_cardemu_state_changed(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + NFC_CALLBACK_CARDEMU_STATE_CHANGED];
	bool unpacked_idle;
	CardemuStateChanged_Callback *callback = (CardemuStateChanged_Callback *)packet;
...
static void nfc_callback_wrapper_p2p_state_changed(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + NFC_CALLBACK_P2P_STATE_CHANGED];
	bool unpacked_idle;
	P2PStateChanged_Callback *callback = (P2PStateChanged_Callback *)packet;
