...
static void ptc_v2_callback_wrapper_temperature(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PTC_V2_CALLBACK_TEMPERATURE];
...
static void ptc_v2_callback_wrapper_resistance(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PTC_V2_CALLBACK_RESISTANCE];
...
static void ptc_v2_callback_wrapper_sensor_connected(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PTC_V2_CALLBACK_SENSOR_CONNECTED];
...
void ptc_v2_create(PTCV2 *ptc_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(ptc_v2, uid, ipcon->p, 2, 0, 0);
