...
static void voltage_current_v2_callback_wrapper_current(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CURRENT_V2_CALLBACK_CURRENT];
...
static void voltage_current_v2_callback_wrapper_voltage(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CURRENT_V2_CALLBACK_VOLTAGE];
...
static void voltage_current_v2_callback_wrapper_power(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CURRENT_V2_CALLBACK_POWER];
...
void voltage_current_v2_create(VoltageCurrentV2 *voltage_current_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(voltage_current_v2, uid, ipcon->p, 2, 0, 0);
