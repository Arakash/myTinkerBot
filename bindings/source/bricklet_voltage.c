...
static void voltage_callback_wrapper_voltage(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CALLBACK_VOLTAGE];
...
static void voltage_callback_wrapper_analog_value(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CALLBACK_ANALOG_VALUE];
...
static void voltage_callback_wrapper_voltage_reached(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CALLBACK_VOLTAGE_REACHED];
...
static void voltage_callback_wrapper_analog_value_reached(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + VOLTAGE_CALLBACK_ANALOG_VALUE_REACHED];
...
void voltage_create(Voltage *voltage, const char *uid, IPConnection *ipcon) {
	...
	device_create(voltage, uid, ipcon->p, 2, 0, 1);
