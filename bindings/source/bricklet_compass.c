...
static void compass_callback_wrapper_heading(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + COMPASS_CALLBACK_HEADING];
	Heading_Callback *callback = (Heading_Callback *)packet;
...
static void compass_callback_wrapper_magnetic_flux_density(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + COMPASS_CALLBACK_MAGNETIC_FLUX_DENSITY];
...
void compass_create(Compass *compass, const char *uid, IPConnection *ipcon) {
	...
	device_create(compass, uid, ipcon->p, 2, 0, 0);
