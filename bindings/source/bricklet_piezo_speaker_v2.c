...
static void piezo_speaker_v2_callback_wrapper_beep_finished(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PIEZO_SPEAKER_V2_CALLBACK_BEEP_FINISHED];
	(void)packet;
...
static void piezo_speaker_v2_callback_wrapper_alarm_finished(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PIEZO_SPEAKER_V2_CALLBACK_ALARM_FINISHED];
	(void)packet;
...
void piezo_speaker_v2_create(PiezoSpeakerV2 *piezo_speaker_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(piezo_speaker_v2, uid, ipcon->p, 2, 0, 0);
