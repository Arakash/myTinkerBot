...
static void imu_callback_wrapper_acceleration(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_ACCELERATION];
...
static void imu_callback_wrapper_magnetic_field(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_MAGNETIC_FIELD];
...
static void imu_callback_wrapper_angular_velocity(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_ANGULAR_VELOCITY];
...
static void imu_callback_wrapper_all_data(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_ALL_DATA];
...
static void imu_callback_wrapper_orientation(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_ORIENTATION];
...
static void imu_callback_wrapper_quaternion(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + IMU_CALLBACK_QUATERNION];
...
void imu_create(IMU *imu, const char *uid, IPConnection *ipcon) {
	...
	device_create(imu, uid, ipcon->p, 2, 0, 4);
