...
static void lcd_16x2_callback_wrapper_button_pressed(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LCD_16X2_CALLBACK_BUTTON_PRESSED];
...
static void lcd_16x2_callback_wrapper_button_released(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LCD_16X2_CALLBACK_BUTTON_RELEASED];
...
void lcd_16x2_create(LCD16x2 *lcd_16x2, const char *uid, IPConnection *ipcon) {
	...
	device_create(lcd_16x2, uid, ipcon->p, 2, 0, 1);
