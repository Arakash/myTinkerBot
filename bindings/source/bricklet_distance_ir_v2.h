 ...
 * \ingroup BrickletDistanceIRV2
 *
 * The period in ms is the period with which the {@link DISTANCE_IR_V2_CALLBACK_DISTANCE} callback is triggered
 ...
 * \ingroup BrickletDistanceIRV2
 *
 * The period in ms is the period with which the {@link DISTANCE_IR_V2_CALLBACK_ANALOG_VALUE} callback is triggered
 * periodically. A value of 0 turns the callback off.
 * 
 * If the `value has to change`-parameter is set to true, the callback is only
 * triggered after the value has changed. If the value didn't change
