...
static void linear_poti_callback_wrapper_position(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LINEAR_POTI_CALLBACK_POSITION];
...
static void linear_poti_callback_wrapper_analog_value(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LINEAR_POTI_CALLBACK_ANALOG_VALUE];
...
static void linear_poti_callback_wrapper_position_reached(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LINEAR_POTI_CALLBACK_POSITION_REACHED];
...
static void linear_poti_callback_wrapper_analog_value_reached(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + LINEAR_POTI_CALLBACK_ANALOG_VALUE_REACHED];
...
void linear_poti_create(LinearPoti *linear_poti, const char *uid, IPConnection *ipcon) {
	...
	device_create(linear_poti, uid, ipcon->p, 2, 0, 1);
