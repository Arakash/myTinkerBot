 ...
 * \ingroup BrickletIndustrialCounter
 *
 * The period in ms is the period with which the {@link INDUSTRIAL_COUNTER_CALLBACK_ALL_COUNTER}
 ...
 * \ingroup BrickletIndustrialCounter
 *
 * The period in ms is the period with which the {@link INDUSTRIAL_COUNTER_CALLBACK_ALL_SIGNAL_DATA}
 * callback is triggered periodically. A value of 0 turns the callback off.
 * 
 * If the `value has to change`-parameter is set to true, the callback is only
 * triggered after the value has changed. If the value didn't change within the
