 ...
 * \ingroup BrickletPTCV2
 *
 * The period in ms is the period with which the {@link PTC_V2_CALLBACK_TEMPERATURE} callback is triggered
 ...
 * \ingroup BrickletPTCV2
 *
 * The period in ms is the period with which the {@link PTC_V2_CALLBACK_RESISTANCE} callback is triggered
 * periodically. A value of 0 turns the callback off.
 * 
 * If the `value has to change`-parameter is set to true, the callback is only
 * triggered after the value has changed. If the value didn't change
