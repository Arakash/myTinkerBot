 ...
 * \ingroup BrickletLaserRangeFinderV2
 *
 * The period in ms is the period with which the {@link LASER_RANGE_FINDER_V2_CALLBACK_DISTANCE} callback is triggered
 ...
 * \ingroup BrickletLaserRangeFinderV2
 *
 * The period in ms is the period with which the {@link LASER_RANGE_FINDER_V2_CALLBACK_VELOCITY} callback is triggered
 * periodically. A value of 0 turns the callback off.
 * 
 * If the `value has to change`-parameter is set to true, the callback is only
 * triggered after the value has changed. If the value didn't change
