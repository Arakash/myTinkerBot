...
static void xmc1400_breakout_callback_wrapper_adc_values(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + XMC1400_BREAKOUT_CALLBACK_ADC_VALUES];
	int i;
	uint16_t aligned_values[8];

	ADCValues_Callback *callback = (ADCValues_Callback *)packet;
...
static void xmc1400_breakout_callback_wrapper_count(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + XMC1400_BREAKOUT_CALLBACK_COUNT];
...
void xmc1400_breakout_create(XMC1400Breakout *xmc1400_breakout, const char *uid, IPConnection *ipcon) {
	...
	device_create(xmc1400_breakout, uid, ipcon->p, 2, 0, 0);
