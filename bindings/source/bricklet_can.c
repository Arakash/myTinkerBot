...
static void can_callback_wrapper_frame_read(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + CAN_CALLBACK_FRAME_READ];
	FrameRead_Callback *callback = (FrameRead_Callback *)packet;
...
void can_create(CAN *can, const char *uid, IPConnection *ipcon) {
	...
	device_create(can, uid, ipcon->p, 2, 0, 0);
