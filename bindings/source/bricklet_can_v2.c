...
static void can_v2_callback_wrapper_frame_read_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + CAN_V2_CALLBACK_FRAME_READ_LOW_LEVEL];
	FrameReadLowLevel_Callback *callback = (FrameReadLowLevel_Callback *)packet;
...
void can_v2_create(CANV2 *can_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(can_v2, uid, ipcon->p, 2, 0, 0);
