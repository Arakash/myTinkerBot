...
static void accelerometer_v2_callback_wrapper_acceleration(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_ACCELERATION];
...
static void accelerometer_v2_callback_wrapper_continuous_acceleration_16_bit(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_CONTINUOUS_ACCELERATION_16_BIT];
	int i;
	int16_t aligned_acceleration[30];

	ContinuousAcceleration16Bit_Callback *callback = (ContinuousAcceleration16Bit_Callback *)packet;
...
static void accelerometer_v2_callback_wrapper_continuous_acceleration_8_bit(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + ACCELEROMETER_V2_CALLBACK_CONTINUOUS_ACCELERATION_8_BIT];
	ContinuousAcceleration8Bit_Callback *callback = (ContinuousAcceleration8Bit_Callback *)packet;
...
void accelerometer_v2_create(AccelerometerV2 *accelerometer_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(accelerometer_v2, uid, ipcon->p, 2, 0, 1);
	...
	device_p->response_expected[ACCELEROMETER_V2_FUNCTION_SET_CONTINUOUS_ACCELERATION_CONFIGURATION] = DEVICE_RESPONSE_EXPECTED_FALSE;
