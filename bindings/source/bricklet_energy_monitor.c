...
static void energy_monitor_callback_wrapper_energy_data(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + ENERGY_MONITOR_CALLBACK_ENERGY_DATA];
	EnergyData_Callback *callback = (EnergyData_Callback *)packet;
...
void energy_monitor_create(EnergyMonitor *energy_monitor, const char *uid, IPConnection *ipcon) {
	...
	device_create(energy_monitor, uid, ipcon->p, 2, 0, 0);
