 ...
 * returned values are averaged and calibrated (see {@link joystick_v2_calibrate}).
 * 
 ...
 * \ingroup BrickletJoystickV2
 *
 * The period in ms is the period with which the {@link JOYSTICK_V2_CALLBACK_POSITION}
 ...
 * \ingroup BrickletJoystickV2
 *
 * The period in ms is the period with which the {@link JOYSTICK_V2_CALLBACK_PRESSED}
 * callback is triggered periodically. A value of 0 turns the callback off.
 * 
 * If the `value has to change`-parameter is set to true, the callback is only
 * triggered after at least one of the values has changed. If the values didn't
