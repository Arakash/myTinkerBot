...
static void dmx_callback_wrapper_frame_started(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_STARTED];
	(void)packet;
...
static void dmx_callback_wrapper_frame_available(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_AVAILABLE];
	FrameAvailable_Callback *callback = (FrameAvailable_Callback *)packet;
...
static void dmx_callback_wrapper_frame_low_level(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_LOW_LEVEL];
	FrameLowLevel_Callback *callback = (FrameLowLevel_Callback *)packet;
...
static void dmx_callback_wrapper_frame_error_count(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + DMX_CALLBACK_FRAME_ERROR_COUNT];
	FrameErrorCount_Callback *callback = (FrameErrorCount_Callback *)packet;
...
void dmx_create(DMX *dmx, const char *uid, IPConnection *ipcon) {
	...
	device_create(dmx, uid, ipcon->p, 2, 0, 0);
