...
static void particulate_matter_callback_wrapper_pm_concentration(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PARTICULATE_MATTER_CALLBACK_PM_CONCENTRATION];
	PMConcentration_Callback *callback = (PMConcentration_Callback *)packet;
...
static void particulate_matter_callback_wrapper_pm_count(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + PARTICULATE_MATTER_CALLBACK_PM_COUNT];
	PMCount_Callback *callback = (PMCount_Callback *)packet;
...
void particulate_matter_create(ParticulateMatter *particulate_matter, const char *uid, IPConnection *ipcon) {
	...
	device_create(particulate_matter, uid, ipcon->p, 2, 0, 0);
