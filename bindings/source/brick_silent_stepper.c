...
static void silent_stepper_callback_wrapper_under_voltage(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_UNDER_VOLTAGE];
...
static void silent_stepper_callback_wrapper_position_reached(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_POSITION_REACHED];
...
static void silent_stepper_callback_wrapper_all_data(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_ALL_DATA];
...
static void silent_stepper_callback_wrapper_new_state(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + SILENT_STEPPER_CALLBACK_NEW_STATE];
...
void silent_stepper_create(SilentStepper *silent_stepper, const char *uid, IPConnection *ipcon) {
	...
	device_create(silent_stepper, uid, ipcon->p, 2, 0, 1);
