...
static void barometer_v2_callback_wrapper_air_pressure(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + BAROMETER_V2_CALLBACK_AIR_PRESSURE];
...
static void barometer_v2_callback_wrapper_altitude(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + BAROMETER_V2_CALLBACK_ALTITUDE];
...
static void barometer_v2_callback_wrapper_temperature(DevicePrivate *device_p, Packet *packet) {
	...
	void *user_data = device_p->registered_callback_user_data[DEVICE_NUM_FUNCTION_IDS + BAROMETER_V2_CALLBACK_TEMPERATURE];
...
void barometer_v2_create(BarometerV2 *barometer_v2, const char *uid, IPConnection *ipcon) {
	...
	device_create(barometer_v2, uid, ipcon->p, 2, 0, 0);
